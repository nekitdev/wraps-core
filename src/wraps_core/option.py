...
[`Option[T]`][wraps_core.option.Option] represents an optional value:
every [`Option[T]`][wraps_core.option.Option] is either [`Some[T]`][wraps_core.option.Some] and
contains a value, or [`Null`][wraps_core.option.Null], and does not.
...
[`Option[T]`][wraps_core.option.Option] is commonly paired with pattern matching to query
the presence of [`Some[T]`][wraps_core.option.Some] value (`T`) and take action,
always accounting for the [`Null`][wraps_core.option.Null] case:
...
Here, we know that [`Null`][wraps_core.option.Null] represents only one case,
that is, attempts to divide by zero. However, when we need to represent multiple errors
from one function, we might want to use [`Result[T, E]`][wraps_core.result.Result] instead,
as described in the [`result`][wraps_core.result] section.
"""
...
from wraps_core.reprs import empty_repr, wrap_repr

__all__ = ("NULL", "Option", "Some", "Null", "is_some", "is_null", "wrap_optional")
...
"""Optional value, expressed as the union of [`Some[T]`][wraps_core.option.Some]
and [`Null`][wraps_core.option.Null].
"""

NULL = Null()
"""The instance of [`Null`][wraps_core.option.Null]."""
...
from wraps_core.early.errors import EarlyOption
from wraps_core.result import Error, Ok, Result
